    ...
    try:
        ...
        if 2 <= nationality_value <= 21:  # 如果 Nationality 值在 2 到 21 之间，使用 model2
            ...
            if prediction[0] == 1:
                ...
                st.markdown("<p style='font-size: 20px;'>4. Course tyoe</p>", unsafe_allow_html=True)
                st.markdown("<p style='font-size: 20px;'>5. Debtor</p>", unsafe_allow_html=True)

                # 使用st.columns()进行居中
                col3, col4, col5 = st.columns([4, 1, 3])  # 中间的列宽度为4，左右两列宽度为1
                with col3:
            ...
            if prediction[0] == 1:
                ...
                # 使用st.columns()进行居中
                col3, col4, col5 = st.columns([3, 1, 3])  # 中间的列宽度为4，左右两列宽度为1
                with col3:
                    # 修改为每行两个特征值（包括特征名和对应的值）
                    rows = []
                    for i in range(0, len(new_data.columns), 2):
