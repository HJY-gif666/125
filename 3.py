...
st.markdown("""
    <h1 style='color: darkblue; text-align: center; font-size: 36px;'>Evaluating and forecasting undergraduate dropouts</h1>
    <div style='text-align: center; font-size: 36px;'>Developed by Dr. Songbo Wang et al., Hubei University of Technology.</div>
    <div style='text-align: center; font-size: 36px;'>Email: Wangsongbo@hbut.edu.cn</div>
...
if st.sidebar.button('Predict'):
    st.markdown("<h1 style='color: blue; font-size: 30px;'>Predicted result:</h1>", unsafe_allow_html=True)
            ...
            if prediction[0] == 1:
                ...
                with col3:
                    ...
                    for i in range(0, len(new_data.columns), 2):
                        ...
                        # Modify the display of feature names and values to ensure proper structure
                        row = [
                            f"<span style='font-size: 20px;'>{feature_name_1}: {value_1}</span>", 
                            f"<span style='font-size: 20px;'>{feature_name_2}: {value_2}</span>" if feature_name_2 else ""
                        ]
                        rows.append(row)
