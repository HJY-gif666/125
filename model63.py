...
if st.sidebar.button('Predict'):
    ...
    # 将特征拆分为两部分显示
    split_index = len(new_data.columns) // 2  # 计算中间的拆分点
    data_part_1 = new_data.iloc[:, :split_index]  # 获取前半部分列
    data_part_2 = new_data.iloc[:, split_index:]  # 获取后半部分列

    # 显示输入特征的表格
    st.write("### Input Features")
    st.dataframe(data_part_1, use_container_width=True)  # 显示前半部分的表格
    st.dataframe(data_part_2, use_container_width=True)  # 显示后半部分的表格
    ...
    try:
        ...
        if 2 <= nationality_value <= 21:  # 如果 Nationality 值在 2 到 21 之间，使用 model2
            ...
            if prediction[0] == 1:
                ...
                with col3:
                    # Displaying the ranking with font size 24px
                    st.markdown("<h2 style='font-size: 24px;'>Impact factors ranking:</h2>", unsafe_allow_html=True)
                    st.markdown("<p style='font-size: 24px;'>1. Second</p>", unsafe_allow_html=True)
                    st.markdown("<p style='font-size: 24px;'>2. First</p>", unsafe_allow_html=True)
                    st.markdown("<p style='font-size: 24px;'>3. Fee</p>", unsafe_allow_html=True)
                    st.markdown("<p style='font-size: 24px;'>4. Course</p>", unsafe_allow_html=True)
