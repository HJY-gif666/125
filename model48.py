...
# 确保文件路径正确，加载图像
image_path = "Domestic and International.jpg"

try:
    # 使用 PIL 打开图像
    image = Image.open(image_path)
    st.image(image, caption="International and Domestic Education", use_column_width=True)
...
if st.sidebar.button('Predict'):
    ...
    try:
        ...
        if 2 <= nationality_value <= 21:  # 如果 Nationality 值在 2 到 21 之间，使用 model2
            ...
            st.write("使用 best_model_int7.9.pkl 的预测结果:", prediction)

            if prediction[0] == 0:
                st.success("预测的目标值：未毕业")
            elif prediction[0] == 1:
                st.success("预测的目标值：已毕业")
            ...
            st.write("使用 best_model_non5.pkl 的预测结果:", prediction)
